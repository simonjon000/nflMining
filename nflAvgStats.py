...
with open('avg.csv', 'w') as f:  # open file so we can write to it
    for year in range(YEARS_START, YEARS_END - 1, -1):  # for each year
    ...
    for year in range(YEARS_START, YEARS_END+1):#for each year
        ...
        while not successful:
            for team in TEAMS:  # for each team
                itr = itr + 1
                # print(team)
                url = "https://www.pro-football-reference.com/teams/%s/%s_roster.htm" % (
                team, year)  # make team/year url
